...
- 安全审核：支持屏蔽词、AI 提示词审核、Moderation 接口图片审核（如模力方舟 nsfw-classifier）、AI 图片审核和审核白名单；WebUI 任务详情可查看每张图的审核分类分数、命中规则和被拦截的图片，生成页提供审核测试工具（不生图直接测提示词/图片审核）。
...
| 启用 Moderation 接口审核 | 生图后、发送前调用 OpenAI 兼容 `/moderations` 接口逐张审核图片；与 AI 图片审核独立，两者都开启时先执行本审核。 |
